...
A static, browser-only web app for creating a six-page Webtezza-style social media performance PDF report. The form starts empty, the PDF uses only the metrics you manually enter, and uploaded images are resized locally before export. Missing metric fields are allowed and display as N/A in the report.
...
- Require only Report Type and Report Month/Week before PDF generation. Metrics are optional.
...
- `script.js` — form behavior, Report Type/Period validation, local image resizing, report rendering, real jsPDF text/table export, and browser print fallback.
