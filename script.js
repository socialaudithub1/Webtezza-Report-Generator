...
function platformPage(platformKey, pageNumber, metrics) {
  ...
  return `
    <section class="report-page">
      ...
      <p class="report-subtitle">${displayValue(reportPeriod.value)}</p>
...
function addPdfCoverHeader(pdf, title, period) {
  ...
  pdf.setFontSize(9);
  pdf.text(period, A4_WIDTH_PT - 40, 58, { align: 'right' });
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(28);
  pdf.text(title, 40, 102);
}

function addPdfPageTitle(pdf, title) {
  pdf.setTextColor(11, 41, 75);
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(20);
  pdf.text(title, 40, 64);
  pdf.setDrawColor(210, 224, 240);
  pdf.line(40, 78, A4_WIDTH_PT - 40, 78);
}
...
function addPlatformPdfPage(pdf, platformKey, pageNumber, metrics, period) {
  const platform = metrics[platformKey];
  addPdfPageTitle(pdf, `${platform.name} Performance`);
...
async function generatePdf() {
  ...
  try {
    ...
    addPdfCoverHeader(pdf, 'Social Media Performance Report', period);
    addPdfFooter(pdf, 1);
    ...
    addPdfPageTitle(pdf, 'Platform Comparison');
    addPdfFooter(pdf, 2);
    ...
    pdf.addPage('a4', 'portrait');
    addPdfPageTitle(pdf, 'Key Insights and Action Plan');
    addPdfFooter(pdf, 6);
    pdf.setTextColor(11, 41, 75);
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(13);
